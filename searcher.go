...
// 09 Metrics
// add metrics to your application to measure the number of cache hits and misses
...
// Searcher is an emoji search engine component. interface & implementation
...
func (searcher *searcher) Search(ctx context.Context, query string) ([]string, error) {
	searcher.Logger(ctx).Debug("Search", "query", query)

	// 07 Cache Component
	// Try to get the emojis from the cache, but continue if it's not found or
	// there is an error.
	if emojis, err := searcher.cache.Get().Get(ctx, query); err != nil {
		searcher.Logger(ctx).Error("cache.Get", "query", query, "err", err)
	...
	if err := searcher.cache.Get().Put(ctx, query, results); err != nil {
		searcher.Logger(ctx).Error("cache.Put", "query", query, "err", err)
	}
...
func (searcher *searcher) SearchChatGPT(ctx context.Context, query string) ([]string, error) {
	...
	completion, err := searcher.chatgpt.Get().Complete(ctx, prompt)
	if err != nil {
		return nil, fmt.Errorf("chatgpt: %w", err)
	}
	searcher.Logger(ctx).Debug("ChatGPT completion", "query", query, "completion", completion)
